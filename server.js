...
async function handleSharedStateUpdate(request, response) {
  ...
  syncCurrentEventSnapshotOnServer(sharedStateStore.state);
...
function applyDataGolfPayloadToSharedState(state, payload) {
  ...
  const inPlayByName = new Map(inPlay.map((player) => [normalizeText(player.name), player]));
  ...
  nextState.livePlayers = liveStats.map((player) => ({
    ...
    thru: player.thru || "",
    teeTime: player.teeTime || inPlayByName.get(normalizeText(player.name))?.teeTime || ""
  }));
...
function normalizeDataGolfPlayer(raw) {
  ...
  const rawThru = pickFirst(raw, ["thru", "holes_completed", "holes", "current_hole", "hole"]);
  const teeTime = normalizeTeeTimeValue(
    pickFirst(raw, [
      "tee_time",
      "teetime",
      "tee",
  ...
  return {
    ...
    thru: normalizeThruValue(rawThru),
    teeTime,
    madeCut: normalizeMadeCutValue(raw),
    money: 0
  };
}
...
function normalizeThruValue(value) {
  const text = String(value || "").trim();
  if (!text || looksLikeTeeTime(text)) {
    return "";
  }
  return text;
}

function normalizeTeeTimeValue(primary, thruFallback) {
  const primaryText = String(primary || "").trim();
  if (primaryText) {
    return primaryText;
  }

  const thruText = String(thruFallback || "").trim();
  if (looksLikeTeeTime(thruText)) {
    return thruText;
  }
...
function looksLikeTeeTime(value) {
  const text = String(value || "").trim().toUpperCase();
  if (!text) {
    return false;
  }

  return /\b(AM|PM)\b/.test(text) || /^\d{1,2}:\d{2}$/.test(text) || /^\d{1,2}:\d{2}\s?(AM|PM)?$/.test(text);
}

function normalizeMadeCutValue(raw) {
  const explicit = pickFirst(raw, ["made_cut", "make_cut", "madecut", "mc", "status", "cut_status", "result"]);
  ...
  if (explicitText) {
    if (["CUT", "MC", "MISSED CUT", "MISSED", "MDF", "WD", "DQ"].includes(explicitText)) {
      return false;
    }

    if (explicitText.includes("MISSED") || explicitText.includes("CUT")) {
      return false;
    }

    if (["MADE", "PLAYING", "ACTIVE", "IN"].includes(explicitText)) {
      return true;
    }
  }

  const positionText = String(pickFirst(raw, ["position", "pos", "rank", "place"]) || "").trim().toUpperCase();
  if (["CUT", "MC", "MDF", "WD", "DQ"].includes(positionText)) {
    return false;
  }
...
function getSharedCutLineScore(state) {
  const tournamentScores = (state.livePlayers || [])
    ...
    .map((player) => normalizeSharedScore(player.score));
...
function syncCurrentEventSnapshotOnServer(state) {
  if (!state || typeof state !== "object" || !state.currentEventId) {
    return;
  }

  state.eventSnapshots = state.eventSnapshots && typeof state.eventSnapshots === "object"
  ...
  state.eventSnapshots[state.currentEventId] = snapshot;
}
