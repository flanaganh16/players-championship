...
function renderScoreChart() {
  ...
  const padding = { top: 14, right: 118, bottom: 28, left: 42 };
  ...
  const chartLines = managers.map((manager, index) => {
    ...
    return `
      <g>
        ...
        <circle cx="${selectedPoint.x.toFixed(1)}" cy="${selectedPoint.y.toFixed(1)}" r="3.5" fill="${color}" stroke="rgba(255,255,255,0.92)" stroke-width="1.8"></circle>
        <text x="${labelX.toFixed(1)}" y="${labelY.toFixed(1)}" fill="${color}" font-size="12" font-weight="700" font-family="Space Grotesk, sans-serif">${escapeHtml(`${manager} ${formatScore(Number(latestPoint.score) || 0)}`)}</text>
      </g>
    `;
  }).join("");
