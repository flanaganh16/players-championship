...
const elements = {
  ...
  chartDayRange: document.getElementById("chartDayRange"),
...
function renderTeams() {
  ...
  boardOrder.forEach((manager, index) => {
    ...
    teamCard.classList.remove("is-collapsed");
    teamBody.classList.remove("is-collapsed");
...
function renderScoreChart() {
  ...
  const chartWidth = isMobileChart ? 620 : 820;
  const chartHeight = isMobileChart ? 360 : 188;
  const padding = isMobileChart
    ? { top: 20, right: 96, bottom: 42, left: 46 }
  ...
  const selectedSnapshotIndex = snapshots.length - 1;
  ...
  const chartLines = managers.map((manager, index) => {
    ...
    const trendEmoji = getChartTrendEmoji(manager, snapshots);
    ...
    const labelFontSize = isMobileChart ? 17 : 13;
    ...
    return `
      <g>
        ...
        <text x="${labelX.toFixed(1)}" y="${labelY.toFixed(1)}" fill="${color}" font-size="${labelFontSize}" font-weight="700" font-family="Space Grotesk, sans-serif">${escapeHtml(`${trendEmoji ? `${trendEmoji} ` : ""}${manager} ${formatScore(Number(latestPoint.score) || 0)}`)}</text>
      </g>
    `;
  }).join("");
  ...
  const selectedLegend = managers.map((manager, index) => {
    ...
    return `
      <span class="score-chart-legend-item">
        ...
        <span class="score-chart-legend-name">${escapeHtml(`${trendEmoji ? `${trendEmoji} ` : ""}${manager}`)}</span>
...
function renderChartDayRange() {
  if (!elements.chartDayRange) {
    return;
  }

  const dgByName = new Map(state.dataGolfPlayers.map((player) => [normalizeName(player.name), player]));
  const todayPlayers = state.livePlayers
    .filter((player) => player.madeCut !== false && hasStartedRound(player))
    .map((player) => ({
      name: player.name,
      score: getPreferredTodayScore(player, dgByName.get(normalizeName(player.name)))
    }))
    .filter((player) => Number.isFinite(player.score));

  if (!todayPlayers.length) {
    elements.chartDayRange.innerHTML = `
      <span class="chart-day-range-low">Today's low score --</span>
      <span class="chart-day-range-high">Today's high score --</span>
    `;
    return;
  }

  const lowPlayer = todayPlayers.reduce((best, player) => {
    if (!best || player.score < best.score) {
      return player;
    }
    if (player.score === best.score) {
      return player.name.localeCompare(best.name) < 0 ? player : best;
    }
    return best;
  }, null);
  const highPlayer = todayPlayers.reduce((best, player) => {
    if (!best || player.score > best.score) {
      return player;
    }
    if (player.score === best.score) {
  ...
  elements.chartDayRange.innerHTML = `
    <span class="chart-day-range-low">Today's low score ${escapeHtml(formatScore(lowPlayer?.score ?? 0))} ${escapeHtml(formatFeedDisplayName(lowPlayer?.name || ""))}</span>
    <span class="chart-day-range-high">Today's high score ${escapeHtml(formatScore(highPlayer?.score ?? 0))} ${escapeHtml(formatFeedDisplayName(highPlayer?.name || ""))}</span>
  `;
}

function getPreferredTodayScore(livePlayer, dataGolfPlayer = null) {
  const dataGolfToday = normalizeScore(dataGolfPlayer?.todayScore);
  if (dataGolfPlayer && Number.isFinite(dataGolfToday)) {
    return dataGolfToday;
  }

  return normalizeScore(livePlayer?.todayScore);
}

function getChartTrendEmoji(manager, snapshots) {
  const recentScores = snapshots
    .slice(-4)
    .map((snapshot) => Number(snapshot?.scores?.[manager]))
  ...
  const deltas = recentScores.slice(1).map((score, index) => score - recentScores[index]);
  const totalChange = recentScores[recentScores.length - 1] - recentScores[0];
  const improvingMoves = deltas.filter((delta) => delta < 0).length;
  const worseningMoves = deltas.filter((delta) => delta > 0).length;
  const standings = getProjectedStandings();
  const teamSummary = standings.find((entry) => entry.manager === manager);
  const countingGolfers = teamSummary?.golfers?.filter((entry) => entry.golfer !== teamSummary?.droppedGolfer) || [];
  const strongestWinProbability = countingGolfers.reduce((best, entry) => {
    const dgPlayer = getDataGolfPlayer(entry.golfer);
    const win = Number(dgPlayer?.win);
    return Number.isFinite(win) ? Math.max(best, win) : best;
  }, 0);
  const stillInContention = strongestWinProbability >= 1;

  if (stillInContention && totalChange <= -2 && worseningMoves === 0 && improvingMoves >= 2) {
    return "🔥";
  }

  if (totalChange >= 2 && improvingMoves === 0 && worseningMoves >= 2) {
    return "🥶";
  }
...
function formatFeedDisplayName(name) {
  const raw = String(name || "").trim();
  if (!raw.includes(",")) {
    return raw;
  }

  const [lastName, firstName] = raw.split(",").map((part) => part.trim()).filter(Boolean);
